"""Tests for signalfx.config — config loading, validation, init wizard."""
...
class TestSignalFXConfig:
    """Validation rules on the Pydantic models."""

    def test_minimal_valid(self) -> None:
        cfg = SignalFXConfig(**_minimal_config_dict())
...
class TestCLI:
    ...
    def test_version_output(self) -> None:
        ...
        from signalfx.cli import cli

        runner = CliRunner()
        result = runner.invoke(cli, ["version"])
        assert result.exit_code == 0
        assert "signalfx" in result.output
    ...
    def test_config_show_missing(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        from signalfx import config as config_mod
        from signalfx.cli import cli
